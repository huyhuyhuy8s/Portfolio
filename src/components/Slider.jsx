...
const Slider = () => {
    ...
    useGSAP((context, contextSafe) => {
        ...
        const loader = contextSafe(() => {
            gsap.set(".slider-title-wrapper, .preview img, .slider-counter>div", { top: "200%" });
            ...
            tl.to(".slider-title-wrapper, .nav-item a, .slider-indicators p, .preview img, .slider-counter>div", {
                top: 0,
                stagger: 0.075,
                duration: 1,
                ease: "power3.out"
    ...
    const handleClick = (event) => {
        ...
        if (sliderPreview.contains(event.target)) {
            ...
            if (clickedPrev) {
                const clickedIndex = Array.from(document.querySelectorAll(".preview")).indexOf(clickedPrev) + 1;
        ...
        if (clickPosition < sliderWidth / 2 && currentImg !== 1) {
            ...
            animateSlide("left");
        } else if (clickPosition > sliderWidth / 2 && currentImg !== sliderConfig.totalSlides) {
            setCurrentImg(prev => prev + 1);
            animateSlide("right");
        }
    };
    ...
    return (
        <div className="slider" ref={sliderRef} onClick={handleClick}>
            <div className="container" ref={containerRef}>
                {Array.from({ length: Math.min(5, sliderConfig.totalSlides) }).map((_, colIndex) => (
                    <div key={colIndex + 1} className={`col c-${colIndex + 1}`}>
                        {Array.from({ length: Math.min(5, sliderConfig.totalSlides) }).map((_, itemIndex) => {
                            // For c-3, we want to show img1 in the third position
                            let imgIndex = itemIndex + 1;
                            if (colIndex === 2 && itemIndex === 2) { // c-3, third item (index 2)
                                imgIndex = 1;
                            }
                            return (
            ...
            <div className="slider-preview">
                {Array.from({ length: sliderConfig.totalSlides }).map((_, index) => (
                    <div key={index + 1} className={`preview ${currentImg === index + 1 ? 'active' : ''}`}>
